...
| B3 | Keyword search | ✅ | |
| B3-HISTORY | Recent-search memory (last 10 terms) | ✅ | One client-only store `src/lib/use-search-history.ts` (localStorage `hatiwal.searchHistory`; trim · min 2 chars · case-insensitive dedupe · newest-first · cap 10 — mobile's `searchHistory.store.ts` rules) rendered by the shared `components/shared/search-history-panel.tsx` from BOTH fields through `components/shared/search-box.tsx`: the header floats it as a dropdown **only while that field has focus** (it overlays the page), the Bazaar sidebar renders it inline **whenever its box is empty** (in flow, covers nothing) — and that sidebar field is `lg`+ only, since below `lg` the header's field is already on screen. Chip re-runs the search through the existing `?q=` path. No API, no auth, nothing new in the URL — works logged out |

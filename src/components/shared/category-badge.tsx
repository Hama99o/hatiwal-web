...
interface CategoryBadgeProps extends VariantProps<typeof categoryBadgeVariants> {
  ...
  asLink?: boolean;
  /**
   * Active listing count for this category. When provided it is rendered
   * alongside the name so a chip is never a blind click — the buyer sees
   * whether there is anything behind it before navigating.
   */
  count?: number;
  className?: string;
}
...
export function CategoryBadge({
  ...
  const content = (
    <span className={cn(categoryBadgeVariants({ size, tone }), className)}>
      ...
      {hasCount &&
        (count > 0 ? (
          ...
          // Never a bare "0" — an empty category says so, in words.
  ...
  if (asLink) {
    return (
      <Link
        ...
        // name has no meaning without it.
        aria-label={
          hasCount
            ? `${name} — ${count > 0 ? t("listing.shopCount", { count }) : t("categoriesPage.noListings")}`
            : name
        }

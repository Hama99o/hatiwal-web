...
# Google Search Console URL-prefix ownership. Build-time like every other
# NEXT_PUBLIC_*: the tag is rendered into the HTML, so it must exist when
# `next build` runs, not merely at container start.
